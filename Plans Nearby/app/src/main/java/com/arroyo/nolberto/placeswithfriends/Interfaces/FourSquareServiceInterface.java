...
public interface FourSquareServiceInterface {
    ...
    @GET("venues/explore/?&venuePhotos=1&client_id="+ BuildConfig.FOURSQUARE_CLIENT_ID+"&client_secret="+BuildConfig.FOURSQUARE_CLIENT_SECRET+"&v=20160913")
    Call<CallBackResult> getCategoryByCity(@Query("near") String near,@Query("section")String section);

    @GET("venues/explore/?&venuePhotos=1&sortByDistance=1&client_id="+ BuildConfig.FOURSQUARE_CLIENT_ID+"&client_secret="+BuildConfig.FOURSQUARE_CLIENT_SECRET+"&v=20160913")
    Call<CallBackResult> getCategoryNearby(@Query("ll") String latLon, @Query("section")String section);
